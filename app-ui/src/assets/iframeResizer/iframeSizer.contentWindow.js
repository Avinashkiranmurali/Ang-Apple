...
function setPointsBalanceListener(e) {
    if (e.detail) {
        if ('parentIFrame' in window) {
            window.parentIFrame.sendMessage({
                type: 'pointsBalance',
                formatted: e.detail.formatted,
                integer: e.detail.integer
            })
        }
    }
}

function setRouteListener(e) {
    if (e.detail) {
        if ('parentIFrame' in window) {
            window.parentIFrame.sendMessage({
                type: 'navigation',
...
function setAppleCartCountListener(e) {
    if (e.detail) {
        if ('parentIFrame' in window) {
            window.parentIFrame.sendMessage({
                type: 'appleCartCount',
...
function setSSOEventListener(e) {
    if (e.detail) {
        if ('parentIFrame' in window) {
            window.parentIFrame.sendMessage({
                type: 'auth',
                value: e.detail.persistentId
            })
        }
    }
}

function setOrderSuccessEventListener(e) {
    if (e.detail) {
        if ('parentIFrame' in window) {
            window.parentIFrame.sendMessage({
                type: 'order',
...
function setSessionTimeoutEventListener(e) {
    if ('parentIFrame' in window) {
        window.parentIFrame.sendMessage({
            type: 'error',
 ...
 */
window.iframeEventsStorage = window.iframeEventsStorage || {};

var routeObj = {};
var eventsMap = [
    { eName: 'setPointsBalance', eFunc: setPointsBalanceListener },
    { eName: 'route', eFunc: setRouteListener },
    { eName: 'setAppleCartCount', eFunc: setAppleCartCountListener },
    { eName: 'setSSO', eFunc: setSSOEventListener },
    { eName: 'orderSuccess', eFunc: setOrderSuccessEventListener },
    { eName: 'sessionTimeoutEvent', eFunc: setSessionTimeoutEventListener }
];

for (var i = 0; i < eventsMap.length; i++) {
    if (!window.iframeEventsStorage[eventsMap[i]['eName']]) {
        window.addEventListener(eventsMap[i]['eName'], eventsMap[i]['eFunc']);
        window.iframeEventsStorage[eventsMap[i]['eName']] = eventsMap[i]['eName'];
    }
}


window.iFrameResizer = {
    // wrap it in this callback to make sure the parentIframe object is registered and available.
    readyCallback: function () {
        var e = new CustomEvent('route', {
            detail: {
        ...
        /**Sent unsupported browser event**/
        if (!navigator.cookieEnabled) {
            setUnsupportedBrowser();
            window.location.href = 'angBrowserUnsupported.jsp';
        }
    }
};
